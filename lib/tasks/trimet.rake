...
namespace :trimet do
  desc "Sets up TriMet main rail data"
  task rail: :environment do
  ...
  desc "Sets up TriMet rail stops into bi-directional pairs"
  task stop_pairs: :environment do
    groups = [
      MAX_BLUE_LINE,
      MAX_RED_LINE,
      MAX_GREEN_LINE,
    ...
    groups.each do |group|
      group.each do |stop_pair|
        ...
        stop_pair.uniq.each do |local_id|
          ...
          stop.update(location_id: location.id)
        end
      end
    end
  end

  task all: %W[rail stop_pairs] do
  end
